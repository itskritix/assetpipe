...
async function handleRequest(request) {
  ...
  if (path === 'assets') {
    const iconParam = searchParams.get('i') || searchParams.get('assets');
    if (!iconParam)
      return new Response("You didn't specify any assets!", { status: 400 });
    ...
    if (isNaN(perLine) || perLine < -1 || perLine > 50)
      return new Response('Assets per line must be a number between 1 and 50', {
        status: 400,
      });
    ...
    if (!iconNames)
      return new Response("You didn't format the assets param correctly!", {
        status: 400,
      });
